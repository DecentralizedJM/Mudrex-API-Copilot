...
def troubleshoot_500_error(context: str) -> str:
    ...
    return (
        ...
        "1. **Invalid SL/TP inline values:** Ensure `slTriggerPrice` and "
        "`tpTriggerPrice` are not negative, do not have excessive decimal "
        "precision, and are on the correct side of the entry price "
        "(SL below entry for longs, above for shorts; TP vice-versa).\n\n"
        "2. **Bracket Order vs Position Management:** Inline SL/TP can "
        "trigger a 500 if the order structure is invalid for an existing "
        "position. Try placing the order *without* SL/TP first, then use "
        "the Set SL/TP endpoint:\n"
        "   `POST /fapi/v1/futures/positions/{id}/riskorder`\n\n"
        "3. **Parameter Precision:** Mudrex expects numeric values as "
        "**strings**. Ensure the precision matches the asset's tick size "
        "(e.g. BTCUSDT uses 1 decimal for price, ETHUSDT uses 2). "
        "Check `GET /fapi/v1/futures/{asset_id}` for `tickSize`.\n\n"
        "4. **Payload Validation:** Verify all required fields are present "
        "(`symbol`, `side`, `type`, `quantity`). A missing or `null` field "
        "can produce a generic 500.\n\n"
        "5. **Retry & Inspect:** If the 500 is intermittent, retry after "
        "1-2 seconds. If it persists, capture the full response body — "
        "Mudrex sometimes includes a `msg` field with details."
    )
